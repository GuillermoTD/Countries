...
export class InputsBarComponent {
  ...
  constructor(private countriesService:CountryApiService, private router:Router){}
  ...
  onSelectChange(option:any){
    ...
    this.countriesService.getCountriesByRegion(option.target.value).subscribe(fetchedCountry=>{
  ...
  SearchCountry(){
    ...
    this.countriesService.getCountriesByName(this.inputText).subscribe(fetchedCountry=>{
      this.fetchedData = fetchedCountry.data
